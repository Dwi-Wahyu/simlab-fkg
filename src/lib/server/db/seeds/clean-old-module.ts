...
async function main() {
	...
	if (!oldMod) {
		console.log('Modul lama tidak ditemukan.');
		await client.end();
		return;
	}
	console.log('Menemukan modul lama: ' + oldMod.name + ' (ID: ' + oldMod.id + ')');
	const assessments = await db.query.practicumAssessment.findMany({
		where: eq(schema.practicumAssessment.moduleId, oldMod.id)
	});
	if (assessments.length > 0) {
		console.log('SKIP: ' + assessments.length + ' assessment aktif terhubung.');
	} else {
		await db
			.delete(schema.practicumLogbookTemplate)
			.where(eq(schema.practicumLogbookTemplate.moduleId, oldMod.id));
		await db
			.delete(schema.practicumModuleCriteria)
			.where(eq(schema.practicumModuleCriteria.moduleId, oldMod.id));
