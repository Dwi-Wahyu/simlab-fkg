...
import { generateAlatPeriodicReport } from '$lib/server/reports/alatPeriodicReport';
...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const filename = `Laporan_Alat_${labSlug}_${filenameTimeSegment}.xlsx`;

	try {
		const buffer = await generateAlatPeriodicReport({
		...
		console.error('Error generating alat periodic report:', err);
		throw error(500, 'Gagal menghasilkan laporan alat.');
	}
};
