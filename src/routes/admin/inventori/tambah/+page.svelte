<script lang="ts">
	...
	import { Upload, X, Search } from '@lucide/svelte';
	...
	let selectedType = $state(''); // Kategori
	let selectedEquipmentType = $state(''); // Jenis
	let selectedUnit = $state(''); // Satuan
	...
	function clearAssetSelection() {
		...
		brand = '';
		description = '';
		// Reset choices as well if needed, or keep them
	}
	...
	// Derived trigger content sesuai dokumentasi
	...
	// Otomatis set Satuan Dasar ke UNIT jika Kategori adalah ASET
	$effect(() => {
	...
	$effect(() => {
		if (form?.success) {
			dialogConfig = {
				type: 'success',
				title: 'Berhasil!',
				description: 'Data inventori baru telah berhasil disimpan ke dalam sistem.',
			...
			>
				...
				<div class="flex flex-col gap-2 md:col-span-2">
					...
					{#if selectedAssetId}
						<p class="text-[10px] text-muted-foreground italic">
							Menggunakan template dari aset yang sudah ada.
						</p>
					{/if}
				</div>
				...
				<Dialog.Root bind:open={isDialogOpen}>
					<Dialog.Content class="sm:max-w-106.25">
						<Dialog.Header>
							<Dialog.Title>Pilih Template Aset</Dialog.Title>
							<Dialog.Description>
								Cari aset yang sudah ada untuk menyamakan penamaan dan data dasar.
							</Dialog.Description>
						</Dialog.Header>
						<div class="space-y-4 py-4">
							...
							<div class="max-h-[300px] overflow-y-auto rounded-md border border-slate-200">
								...
								{:else}
									<div class="p-4 text-center text-sm text-slate-500">Aset tidak ditemukan.</div>
								{/if}
							</div>
						</div>
					</Dialog.Content>
				...
				<div class="flex flex-col gap-2 md:col-span-2">
					...
					<Input type="text" name="location" id="location" placeholder="Contoh: Rak B-12" />
				</div>

				<!-- Satuan Dasar -->
				<div class="flex flex-col gap-2">
					<Label for="baseUnit">Satuan Dasar</Label>
					<Select.Root type="single" bind:value={selectedUnit} disabled={selectedType === 'ASSET'}>
						<Select.Trigger class="w-full text-left">
							{unitTrigger}
						</Select.Trigger>
						<Select.Content>
				...
				{#if selectedType === 'ASSET'}
					<div class="flex animate-in flex-col gap-2 duration-300 fade-in slide-in-from-top-2">
						<Label for="serialNumber">Nomor Seri</Label>
					...
					<div class="flex animate-in flex-col gap-2 duration-300 fade-in slide-in-from-top-2">
						...
						<input type="hidden" name="laboratoriumId" value={selectedLab} />
					</div>
					<div class="flex animate-in flex-col gap-2 duration-300 fade-in slide-in-from-top-2">
