<script lang="ts">
	...
	// Initial class IDs (multi-class support with single class fallback)
	const getScheduleClassIds = (s: any) => {
		if (s.classes && s.classes.length > 0) {
			return s.classes.map((c: any) => c.classId);
		}
		return s.classId ? [s.classId] : [];
	};

	// Form State
	let title = $state(untrack(() => data.schedule.title));
	let selectedSeriesId = $state(untrack(() => data.schedule.seriesId || ''));
	let selectedBatch = $state('');
	let selectedClassIds = $state<string[]>(untrack(() => getScheduleClassIds(data.schedule)));
	...
	let dateStr = $state(untrack(() => initialDate));
	let startTimeStr = $state(untrack(() => initialStartTime));
	let endTimeStr = $state(untrack(() => initialEndTime));

	// Step 2 Form State
	let selectedType = $state(untrack(() => data.schedule.type || 'PRAKTIKUM'));
	let notes = $state(untrack(() => data.schedule.notes || ''));
	...
	<Dialog.Content class="scrollbar-elegant max-h-[85vh] max-w-2xl overflow-y-auto p-6">
		<Dialog.Header>
			...
			<Dialog.Description class="text-sm text-slate-500">
				Alur langkah demi langkah untuk mengedit jadwal kegiatan secara lengkap.
			</Dialog.Description>
		</Dialog.Header>
				...
				<Card.Header class="border-b bg-slate-50/50 pb-4">
					<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						<div>
							...
							<p class="text-sm text-slate-500">Perbarui detail jadwal kegiatan praktikum.</p>
						</div>
