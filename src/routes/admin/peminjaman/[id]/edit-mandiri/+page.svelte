<script lang="ts">
	...
	$effect(() => {
		if (data.currentSelectedItems && selectedItems.length === 0) {
			selectedItems = data.currentSelectedItems.map((item) => {
				const availableItem = data.items.find((i) => i.id === item.itemId);
				const maxStock = availableItem ? availableItem.equipments?.length || 0 : item.qty;
				return {
					itemId: item.itemId!,
					name: item.name,
					qty: item.qty,
					maxStock: Math.max(item.qty, maxStock)
				};
			});
		}
	});
	...
	function showErrorNotification(message: string) {
		...
		notificationDescription = message || 'Terjadi kesalahan saat menyimpan pengajuan.';
				...
				<Card.Content class="space-y-4 pt-6 sm:pt-0">
					...
					<div class="space-y-3 pt-2">
						<div class="space-y-1.5">
							<Label for="surat"
								>Surat Permohonan (PDF/DOCX, maks 10MB) <span class="font-normal text-slate-400"
									>(Biarkan kosong untuk mempertahankan surat lama)</span
								></Label
							>
							<Input
								id="surat"
							...
							{#if data.lending.surat}
								<div class="mt-1 text-xs text-muted-foreground">
									...
									<a
										...
										target="_blank"
										class="font-semibold text-[#006a34] hover:underline"
									>
										Lihat Surat Lama
									</a>
								</div>
