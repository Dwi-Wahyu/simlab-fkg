<script lang="ts">
	...
	import { Search, Plus, Pencil, Trash2, ArrowRightLeft, Package, Ellipsis } from '@lucide/svelte';
...
<form
	...
	use:enhance={() => {
		...
		return ({ result }) => {
			...
			if (result.type === 'success') {
				notificationMsg = 'Sukses mutasi stok.';
				notificationType = 'success';
				notificationOpen = true;
			}
		};
...
<ConfirmationDialog
	...
	onAction={() => {
		const deleteForm: HTMLFormElement | null = document.getElementById(
			'delete-form'
		) as HTMLFormElement;
...
<ConfirmationDialog
	...
	onAction={() => {
		const mutateForm: HTMLFormElement | null = document.getElementById(
			'mutate-form'
		) as HTMLFormElement;
