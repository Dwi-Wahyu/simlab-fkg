<script lang="ts">
	...
	let selectedMaintenanceId = $state(data.cost.maintenanceId || 'none');
	...
	let fileName = $state(data.cost.attachmentName || '');
	let fileSize = $state('');
	let filePreviewUrl = $state<string | null>(data.cost.attachmentPath || null);
...
<div class="mx-auto max-w-4xl space-y-8 p-6">
	...
	<div class="flex items-center gap-4">
		...
		<div>
			<h1 class="text-2xl font-bold text-slate-900">Edit Analisis Biaya</h1>
		...
		<Card.Content class="p-8">
			...
			>
				...
				<div class="space-y-2">
					...
					<input type="hidden" name="existingAttachmentPath" value={data.cost.attachmentPath} />
					<input type="hidden" name="existingAttachmentName" value={data.cost.attachmentName} />
				</div>
...
<Dialog.Root bind:open={isNoteDialogOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title>Edit Catatan Item</Dialog.Title>
		</Dialog.Header>
		<div class="py-4">
...
<NotificationDialog
	...
	description="Data analisis biaya telah berhasil diperbarui."
