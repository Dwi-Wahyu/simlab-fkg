...
import {
  auditLog, equipment, item, maintenance, lending, approval,
  wasteLog, safetyIncident, inventoryReport, auditChecklist,
  practicumSchedule, practicumAssessment, practicumLogbook, practicumSeries,
  practicumScheduleInstructor, lendingItem, practicumLogbookGeneration
...
export const GET: RequestHandler = async ({ params, locals }) => {
  ...
  switch (role) {
    case 'superadmin': {
      ...
      const [baikResult] = await db.select({ value: count() }).from(equipment).where(eq(equipment.condition, 'BAIK'));
      const [ringanResult] = await db.select({ value: count() }).from(equipment).where(eq(equipment.condition, 'RUSAK_RINGAN'));
      const [beratResult] = await db.select({ value: count() }).from(equipment).where(eq(equipment.condition, 'RUSAK_BERAT'));
      const [totalEquipResult] = await db.select({ value: count() }).from(equipment);

      const [incidentsResult] = await db.select({ value: count() }).from(safetyIncident)
    ...
    case 'koordinator': {
      ...
      const [activeLendingsResult] = await db.select({ value: count() }).from(lending)
        .where(eq(lending.status, 'DIPINJAM'));

      const [pendingApprovalsResult] = await db.select({ value: count() }).from(approval)
        .where(eq(approval.status, 'PENDING'));

      const [maintenanceAlertsResult] = await db.select({ value: count() }).from(maintenance)
        .where(eq(maintenance.status, 'PENDING'));

      const [wasteLogsResult] = await db.select({ value: count() }).from(wasteLog)
    ...
    case 'kepalaLab': {
      ...
      const [pendingMaintResult] = await db.select({ value: count() }).from(approval)
      ...
      return json({
        ...
        data: {
          ...
          latestInventoryReport: latestReport
            ? { id: latestReport.id, status: latestReport.status, createdAt: latestReport.createdAt }
            : null
        }
      });
    }
    ...
    case 'instruktur': {
      ...
      const [schedulesThisMonth] = await db.select({ value: count() })
    ...
    case 'peneliti': {
      ...
      const activeLendingsList = await db.query.lending.findMany({
        where: (l, { eq, and, ne }) =>
          and(eq(l.requestedBy, userId), ne(l.status, 'RETURNED')),
        limit: 5,
      ...
      const [totalBorrowed] = await db.select({ value: count() }).from(lending)
    ...
    case 'teknisi': {
      const pendingMaintenanceList = await db.query.maintenance.findMany({
        where: (m, { or, eq }) =>
          or(eq(m.status, 'PENDING'), eq(m.status, 'IN_PROGRESS')),
      ...
      const upcomingCalibrations = await db.query.maintenance.findMany({
        where: (m, { and, eq, gte }) =>
          and(eq(m.maintenanceType, 'KALIBRASI'), eq(m.status, 'PENDING'),
      ...
      const [completedResult] = await db.select({ value: count() }).from(maintenance)
        .where(and(eq(maintenance.status, 'COMPLETED'), gte(maintenance.completionDate, startOfMonth)));
    ...
    case 'spmi': {
      ...
      const [calibratedResult] = await db.select({ value: count() }).from(maintenance)
        .where(and(eq(maintenance.maintenanceType, 'KALIBRASI'), eq(maintenance.status, 'COMPLETED')));
