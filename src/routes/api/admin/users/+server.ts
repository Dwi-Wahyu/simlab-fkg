...
import { count, eq, or, like, desc, and } from 'drizzle-orm';
...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	let whereClause = eq(user.isDeleted, false);
	if (search) {
		whereClause = and(
	...
	const [totalItemsResult] = await db.select({ value: count() }).from(user).where(whereClause);
