...
- **Changes:**
  ...
  - Included a reactive status badge in the UI displaying:
    - _Tidak Kedaluwarsa_ (if no expiry date).
    - _Kedaluwarsa_ (if date is in the past).
    - _X hari lagi_ (if expiring soon; colored red if <= 3 days, default/green if <= 14 days, secondary/gray if > 14 days).
